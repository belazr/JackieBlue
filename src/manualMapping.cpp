...
namespace manMap {
	...
	bool inject(HANDLE hProc, const char* dllPath) {
		...
		if (!launch::createRemoteThread(hProc, reinterpret_cast<launch::tLaunchFunc>(pShellCode), pImageBase, &pModBase)) {
			io::printWinError("Failed to create and execute thread in target process.");
			VirtualFreeEx(hProc, pImageBase, 0, MEM_RELEASE);
